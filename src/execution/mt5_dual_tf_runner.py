        ...
        for s in symbols:
            ...
            if current == 0:
                if (nowt_loop - last_close_time[s]).total_seconds() / 60.0 >= cooldown_min:
                    # Strict RSI Filter to ensure we only trade on strong overbought/oversold signals
                    rsi_val = last_feats["rsi14"].iloc[-1]
                    if raw_side > 0 and p_up >= enter_thresh and rsi_val < 35:
                        desired = +1
                    elif raw_side < 0 and p_down >= enter_thresh and rsi_val > 65:
                        desired = -1
